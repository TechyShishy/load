...
describe('SLAMeter', () => {
  ...
  it('marks the correct number of dots red when slaCount > 0', () => {
    const { container } = render(<SLAMeter slaCount={2} />);
  ...
  it('marks all dots red when slaCount equals MAX_SLA_FAILURES', () => {
    const { container } = render(<SLAMeter slaCount={MAX_SLA_FAILURES} />);
    const redDots = container.querySelectorAll('.bg-red-500');
    expect(redDots).toHaveLength(MAX_SLA_FAILURES);
  });

  it('applies red text to the fraction when slaCount >= 2', () => {
    const { getByText } = render(<SLAMeter slaCount={2} />);
    const fraction = getByText(`2/${MAX_SLA_FAILURES}`);
    expect(fraction).toHaveClass('text-red-400');
  });

  it('applies gray text to the fraction when slaCount < 2', () => {
    const { getByText } = render(<SLAMeter slaCount={1} />);
