...
const ALL_PHASES = ['draw', 'scheduling', 'crisis', 'resolution', 'end'];
const noop = vi.fn();

describe('PhaseIndicator', () => {
  it('renders day and week for round 1', () => {
    const { getByText } = render(<PhaseIndicator onOpenCalendar={noop} currentPhase="draw" round={1} />);
    expect(getByText('Mon, W1')).toBeInTheDocument();
  });

  it('renders day and week for round 6 (Saturday)', () => {
    const { getByText } = render(<PhaseIndicator onOpenCalendar={noop} currentPhase="crisis" round={6} />);
    expect(getByText('Sat, W1')).toBeInTheDocument();
  });

  it('renders day and week for round 8 (Monday week 2)', () => {
    const { getByText } = render(<PhaseIndicator onOpenCalendar={noop} currentPhase="scheduling" round={8} />);
  ...
  it('does not show Weekend badge on weekday rounds', () => {
    const { queryByText } = render(<PhaseIndicator onOpenCalendar={noop} currentPhase="scheduling" round={1} />);
  ...
  it('highlights the active phase with cyan background', () => {
    const { getByText } = render(<PhaseIndicator onOpenCalendar={noop} currentPhase="crisis" round={3} />);
  ...
  it.each(ALL_PHASES)('exactly one pill is active when phase is %s', (phase) => {
    const { container } = render(<PhaseIndicator onOpenCalendar={noop} currentPhase={phase} round={1} />);
  ...
  it('announces day and week in aria-live region', () => {
    const { getByRole } = render(<PhaseIndicator onOpenCalendar={noop} currentPhase="scheduling" round={5} />);
    const status = getByRole('status');
    expect(status.textContent).toBe('Fri, Week 1 – Schedule phase');
  });

  it('calls onOpenCalendar when day button is clicked', () => {
    const onOpenCalendar = vi.fn();
    const { getByRole } = render(<PhaseIndicator onOpenCalendar={onOpenCalendar} currentPhase="scheduling" round={3} />);
    fireEvent.click(getByRole('button', { name: 'Open round calendar' }));
